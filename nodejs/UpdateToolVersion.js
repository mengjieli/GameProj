        ...
        if (message.type == "binary") {
            ...
            switch (cmd) {
                case 0:
                    ...
                    if (cmd == 1) {
                        var bytes = new VByteArray();
                        bytes.writeUIntV(20002);
                        bytes.writeUIntV(0);
                        bytes.writeUTFV(localConfig.version);
                        this.sendData(bytes);
                    } else if (cmd == 20002) {
                        var bytes = new VByteArray();
                        bytes.writeUIntV(20000);
                        bytes.writeUIntV(0);
                        this.sendData(bytes);
                    }
                    break;
                case 20001:
                    bytes.readUIntV();
                    var version = bytes.readUTFV();
                    console.log("update success, current version is: " + version);
                    this.close();
                    break;
...
function start() {
    ...
    var content = (new File("./clients/config.json")).readContent();
    cfg = JSON.parse(content);


    content = (new File("./../flower.json")).readContent();
